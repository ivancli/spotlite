@if(!is_null($productFamilies))
    @foreach($productFamilies as $productFamily)
        <div class="box box-default box-solid product-container {{isset($chosenAPIProductID) && $productFamily->product->id == $chosenAPIProductID ? 'chosen': ''}} {{old("api_product_id") == $productFamily->product->id ? "selected" : ""}}"
             ...
             style="margin-left: -2px;margin-right: -2px;border-radius: 0;"
             data-price="{{$productFamily->product->price_in_cents}}"
             data-component-id="{{$productFamily->component->id}}">
            <div class="box-header with-border">
                <h3 class="box-title"
                    style="text-transform: uppercase; padding-right: 0; font-weight: bold; color: #fff;">{{$productFamily->product->name}}</h3>
            </div>
        ...
        {{--data-family-id="{{$productFamily->id}}"--}}
        {{--data-id="{{$productFamily->product->id}}"--}}
        {{--style="border: 1px solid lightgrey; border-radius: 20px;margin-left: -2px;margin-right: -2px;"--}}
        {{--data-price="{{$productFamily->product->price_in_cents}}"--}}
        {{--data-component-id="{{$productFamily->component->id}}">--}}
